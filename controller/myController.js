...
module.exports = function(app){
  ...
  app.post('/login',(req,res)=>{
    ...
    userAdmin.findOne({idNumber: req.body.username, password: encrypted },(err, user)=>{
      if (!user){
       ...
       {accountValidate: req.session.loginError});
        ...
        req.session.teachingStatus = user.teachingStatus;
        ...
        reportFilter.find({user: req.body.username}, (err, user4)=>{
          strandAdmin.find({}, (err, user5)=>{
            sectionAdmin.find({strand: user5[0].strands}, (err, user6)=>{
              if (user4.length === 0){
                new reportFilter({
                  user: req.body.username,
                  gradeLevel: 'grade 11',
                  strands: user5[0].strands,
                  section: user6[0].section
                }).save((err)=>{
                  console.log('save report filter');
                });
              };
            });
          });
  ...
  app.post('/update-subject-data', (req, res)=>{
   ...
   schedAdmin.find({subject: req.body.courseTitle}, (err, user)=>{
     schedAdmin.updateMany({subject: req.body.idHolder}, {$set:{
       subject: req.body.courseTitle
     }}, (err)=>{
       if (err) throw err;
     });
  });
   ...
   teacherAdmin.find({subject: req.body.courseTitle}, (err, user)=>{
     teacherAdmin.updateMany({subject: req.body.idHolder}, {$set:{
       subject: req.body.courseTitle
     }}, (err)=>{
       if (err) throw err;
     });
...
app.post('/edit-password-settings',(req, res)=>{
  ...
  var algorithm = 'aes256';
  var key = keys.cipherKey.key;
  var text = req.body.oldPassword;

  var cipher = crypto.createCipher(algorithm, key);
  var encrypted = cipher.update(text, 'utf8', 'hex') + cipher.final('hex');


  userAdmin.find({idNumber: req.session.username, password: encrypted},(err, user)=>{

    if(user.length===1){

      var updatedText = req.body.newPassword;

      var cipher2 = crypto.createCipher(algorithm, key);
      var encryptedPw = cipher2.update(updatedText, 'utf8', 'hex') + cipher2.final('hex');
...
app.post('/enroll-student-success', (req, res)=>{
  if (req.body.statusStudent==='forUpdate'){
    enrollStudent.updateOne({lrn: req.body.lrn} ,{$set:{
      ...
      age: req.body.age,
      gender: req.body.gender,
      ipCommunity: req.body.ipCommunity,
      ...
      hiddenEnrollYear: req.body.hiddenEnrollYear,
      hiddenEnrollSem: req.body.hiddenEnrollSem,
      batch: req.body.batch
    }}, (err)=>{
      if (err) throw err;
      console.log('data updated successfully');
      pendingNewStudent.deleteMany({lrn: req.body.lrn}, (err)=>
      {
        if(err)throw err;
      });
    });

  }else if (req.body.statusStudent==='forSave'){
    enrollStudent.find({lrn: req.body.lrn},(err, user)=>{
      if(user.length > 0){
        console.log("Student Already Have a Data");
      }else{
    new enrollStudent({
      lrn: req.body.lrn,
      fullname: req.body.lastname + '/' + req.body.firstname + '/' + req.body.middlename,
  ...
  app.post('/update-student-enroll-status', (req, res)=>{
    filterStudent.updateOne({key: '092as9sadf7898asdk'}, {$set:{
      viewStudents: req.body.viewStudents
    }}, (err)=>{
      console.log('enroll stats updated');
    });
  });

  app.post('/filter-report-update', (req, res)=>{
    reportFilter.updateOne({user: req.session.username}, {$set:{
      strands: req.body.strand,
      section: req.body.section
    }}, (err)=>{
      console.log('enroll report updated');
    });
  });


  app.post('/incomplete-requirements', (req, res)=>{
    tblRequirements.find({lrn: req.body.lrn}, (err, user)=>{
      ...
      if (user.length === 0){
        ...
        tblRequirements.updateOne({lrn: req.body.lrn},{$set:{
          requirements: req.body.requirements
        }}, (err)=>{
          console.log('data updated');
        });
  ...
  app.post('/students-requirements-update', (req, res)=>{
    console.log(req.session.studentRequirements);
    tblRequirements.updateOne({_id: req.session.studentRequirements},{$set:{
      requirements: req.body.requirements
    }}, (err)=>{
      console.log('data edit updated');
    });
  });
  ...
  app.post('/update-schedule', (req, res)=>{
    ...
    schedAdmin.updateOne({_id: req.body.schedId},{$set:{
      timeIn: req.body.timeIn,
      timeOut: req.body.timeOut,
      day: days,
      days: days.slice(1)
    ...
    teacherAdmin.updateOne({
      strand: req.body.strands,
      timeIn: req.body.oldTimeIn,
      timeOut: req.body.oldTimeOut,
      day: req.body.oldDays
    }, {$set:{
      timeIn: req.body.timeIn,
      timeOut: req.body.timeOut,
      day: days,
