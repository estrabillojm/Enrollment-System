...
routerPDF.post('/print-schedule', (req, res, next)=>{
  var externalDataRetrievedFromServer = [];
    schedAdmin.find({strand: req.session.scheduleSubject}, (err, user2)=>{

      user2.forEach(function(myData){

      externalDataRetrievedFromServer.push(myData);
    });
    ...
    var documentDefinition = {
        content: [
          ...
          { text:'\n' + user2[0].strand.toUpperCase() + ' SCHEDULE\n\n' ,style: 'header', fontSize: 11, alignment: 'center', bold: true},
          table(externalDataRetrievedFromServer, ['days','subject','timeIn','timeOut'])
    ...
    pdfDoc.getBase64((data)=>{
        res.writeHead(200,
        {
            ...
            'Content-Disposition':'attachment;filename='+ user2[0].strand +'_schedule.pdf'
        });
...
routerPDF.post('/report-students-pdf', (req, res, next)=>{
  var externalDataRetrievedFromServer = [];
  ...
  enrollmentSettings.find({_id:'5d5eac152b3cd10e8c764ef0'}, (err, user5)=>{
    ...
    reportFilter.find({user: req.session.username}, (err, user)=>{

      enrollStudent.find({
        strand: user[0].strands ,
        section: user[0].section,
        hiddenEnrollSem: user5[0].semester,
        hiddenEnrollYear:user5[0].academicFirst
      }, (err, user2)=>{
      ...
      var documentDefinition = {
          content: [
            ...
            { text: user2[0].strand.toUpperCase() + '-' + user2[0].section.toUpperCase() + '\n\n', style: 'header', fontSize: 11, alignment: 'center', bold: true},
            table(externalDataRetrievedFromServer, ['fullname']),
            {
              text: "\n" + user2.length + " student/s in total"
            }
...
routerPDF.post('/schedule', (req, res, next)=>{



  var externalDataRetrievedFromServer = [];

  teacherAdmin.find({teacher: req.session.username}, (err, user)=>{
    userAdmin.find({idNumber: req.session.username}, (err, user2)=>{

      user.forEach(function(myData){
      externalDataRetrievedFromServer.push(myData);
    });
    ...
    var documentDefinition = {
        content: [
          ...
          { text: '\n' + user2[0].firstname.toUpperCase() + " " + user2[0].lastname.toUpperCase() + "'S SCHEDULE\n\n" , style: 'header',alignment: 'center', fontSize: 11, bold: true},
          table(externalDataRetrievedFromServer, ['days' , 'subject', 'timeIn', 'timeOut','strand'])
    ...
    pdfDoc.getBase64((data)=>{
        ...
        {
            'Content-Type': 'application/pdf',
            'Content-Disposition':'attachment;filename='+ user2[0].firstname + " " + user2[0].lastname + "'s schedule" +'.pdf'
        });
          ...
          if(req.session.studentClassification ==='new student'){
            if (user.length === 1){
              ...
              var documentDefinition = {
                  ...
                  {
                    qr: 'LRN: ' + user[0].lrn + '\nFULLNAME: ' + user[0].firstname + '_' + user[0].lastname,  fit:70, alignment: 'right', margin:[0, 75, 0, 0]
                  },
                  {
...

                  {
                    columns:[
                      ...
                      {
                        qr: 'LRN: ' + user[0].lrn + '\nFULLNAME: ' + user[0].firstname + '_' + user[0].lastname,  fit:70, alignment: 'right', margin:[0, 125, 0, 0]
                      },
